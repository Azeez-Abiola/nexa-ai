...
exports.conversationRouter.get("/", auth_1.authMiddleware, async (req, res) => {
    try {
        let userConversations = await Conversation_1.Conversation.findOne({ userId: req.userId });
        // If user has no document yet, create one
        if (!userConversations) {
            userConversations = new Conversation_1.Conversation({
                userId: req.userId,
...
exports.conversationRouter.post("/", auth_1.authMiddleware, async (req, res) => {
    try {
        let userConversations = await Conversation_1.Conversation.findOne({ userId: req.userId });
        ...
        userConversations.conversationGroups.push(newGroup);
        await userConversations.save();
...
exports.conversationRouter.get("/:id", auth_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const userConversations = await Conversation_1.Conversation.findOne({ userId: req.userId });
        if (!userConversations) {
            return res.status(404).json({ error: "Conversation not found" });
        }
        const group = userConversations.conversationGroups.find((g) => g._id.toString() === id);
        if (!group) {
            return res.status(404).json({ error: "Conversation not found" });
        }
        const conversation = {
...
exports.conversationRouter.delete("/:id", auth_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const userConversations = await Conversation_1.Conversation.findOne({ userId: req.userId });
        if (!userConversations) {
        ...
        const indexToDelete = userConversations.conversationGroups.findIndex((g) => g._id.toString() === id);
        if (indexToDelete === -1) {
            return res.status(404).json({ error: "Conversation not found" });
        }
...
exports.conversationRouter.put("/:id", auth_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
...
exports.conversationRouter.post("/:id/message", auth_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const userMessage = { role: "user", content: content.trim(), timestamp: new Date() };
...
Be helpful and professional.`;
            ...
            aiResponse = await (0, openaiService_1.generateAIResponse)(content, hasPolicies ? policies.map((p) => ({
                title: p.title,
                category: p.category,
                content: p.content,
                score: p.score || 0,
        ...
        if (group.messages.length === 2) {
            try {
                ...
                group.title = await (0, openaiService_1.generateConversationTitle)(content);
            }
            catch (error) {
...
exports.conversationRouter.post("/:id/message/:index/edit", auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const editedMessage = { role: "user", content: content.trim(), timestamp: new Date() };
...
Inform the user politely that you don't have information on this topic, and suggest they contact HR & Compliance.`;
            ...
            aiResponse = await (0, openaiService_1.generateAIResponse)(content, policyContext, group.messages
                .slice(0, -1)
                .map((m) => ({
                role: m.role,
                content: m.content,
...
exports.conversationRouter.post("/:id/message-stream", auth_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const { content } = req.body;
        if (!content || !content.trim()) {
...
Inform the user politely that you don't have information on this topic, and suggest they contact HR & Compliance.`;
        ...
        try {
            // Stream the response
            const generator = (0, openaiService_1.streamAIResponse)(content, policyContext, group.messages
                .slice(0, -1)
                .map((m) => ({
                role: m.role,
                content: m.content,
